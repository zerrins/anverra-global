...
public class Branch {
    ...
    public Branch(UUID id, UUID dealerId, String name, OrganizationStatus status, Long version) {
        if (id == null) {
            throw new IllegalArgumentException("id cannot be null");
        }
        if (dealerId == null) {
            throw new IllegalArgumentException("dealerId cannot be null");
        }
    ...
    public static Branch create(UUID dealerId, String name) {
        return new Branch(UUID.randomUUID(), dealerId, name, OrganizationStatus.ACTIVE, 0L);
    }
